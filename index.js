...
async function createUnitTest(
  componentPath,
  { dry, template, verbose, v, name, n, testDir, d, omitTests, o },
) {
  ...
  require('child_process').fork(
    path.resolve(`${__dirname}/node_modules/hygen/dist/bin`),
    hygenArgs,
 ...
 * cli
 */
const scriptArgs = require('yargs-parser')(process.argv.slice(2));
...
if (scriptArgs && scriptArgs['_'].length) {
  scriptArgs['_'].forEach((filePath) => {
    try {
      createUnitTest(path.normalize(filePath), scriptArgs);
    } catch (err) {
      console.error(err);
    }
  });
